...
esp_err_t camera_raw(TFT_t *dev, uint8_t *out) {
    ...
    for (int y = 0; y < imageHeight; y++) {
        ...
        lcd_draw_multi_pixels(dev, 0, y, imageWidth, colors);
    }
...
void ST7789(void *pvParameters) {
    ...
    spi_master_init(&dev,
                    ...
                    CONFIG_RESET_GPIO,
                    CONFIG_BL_GPIO);
    lcd_init(&dev, CONFIG_WIDTH, CONFIG_HEIGHT, CONFIG_OFFSETX, CONFIG_OFFSETY);

    lcd_fill_screen(&dev, BLACK);

    uint8_t *out = malloc(sizeof(uint8_t) * CONFIG_HEIGHT * CONFIG_WIDTH);
    while (true) {
